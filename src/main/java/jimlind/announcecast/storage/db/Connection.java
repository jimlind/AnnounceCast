...
public class Connection {
  ...
  public Connection() {
    try {
      ...
      statement.execute("CREATE INDEX idx_channels_feed_id ON channels(feed_id)");
      statement.execute(
          "CREATE TABLE IF NOT EXISTS feeds (id TEXT PRIMARY KEY, url TEXT UNIQUE, title TEXT)");
      statement.execute("CREATE INDEX idx_feeds_id ON feeds(id)");
      statement.execute(
          "CREATE TABLE IF NOT EXISTS posted (feed_id TEXT PRIMARY KEY UNIQUE, guid TEXT)");
      statement.execute("CREATE UNIQUE INDEX idx_posted_feed_id ON posted(feed_id)");
      statement.execute(
          "CREATE TABLE IF NOT EXISTS subscriber (feed_id TEXT PRIMARY KEY UNIQUE, active INTEGER NOT NULL CHECK (active IN (0, 1)))");
      statement.execute("CREATE UNIQUE INDEX idx_subscriber_feed_id ON subscriber(feed_id)");
